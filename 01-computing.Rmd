...
For example, you can have a script to pull a Docker image and create some directories in your head node with a script `head-node-setup-env.sh`
...
- Log onto cluster. You can directly ssh to the head node, but the compute nodes are only accessible from the head node, not from the Internet.
```
